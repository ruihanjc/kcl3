...
def lexer() = {
  val string1 = "aaa"
  println(s"test: $string1")
  val rex1 = NTIMES(ALT("a", ONE), 3)
  println(lex_simp(rex1, string1.toList))
}
...
@main
def test5() = {
  println("lexing prog5")
  println(escape(lexing_simp(WHILE_REGS, prog5)).mkString("\n"))
}
...
def email() = {
  ...
  val NAME = RECD("name", PLUS2(RANGE2(lower ++ "_.-")))
  val DOMAIN = RECD("domain", PLUS2(RANGE2(lower ++ "-")))
  val RE = RANGE2(lower ++ ".")
